...
function BundleDetail() {
    ...
    const getPriceDetails = () => {
        ...
        const mode = bundle.bundleMode.find(mode => mode._id === selectedMode);
        if (mode) {
            return {
    ...
    const handleAddToCart = (bundle) => {
        ...
        let cartItems = JSON.parse(sessionStorage.getItem('bundle')) || [];
        const productIndex = cartItems.findIndex(item => item.product._id || item.bundle._id === bundle._id);
    
        if (productIndex !== -1) {
            toast.success('Bundle is already in the cart. Updating the quantity.');
    
            cartItems[productIndex] = {
                ...cartItems[productIndex],
                quantity: 1,
                isCourseBundle:true,
                price: bundle.price || bundle.bundleStartingPrice, // Use bundleStartingPrice as fallback
                mode: selectedMode,
            };
        } else {
            const newCartItem = {
                bundle, // Use bundle instead of product
                quantity: 1,
                isCourseBundle:true,
                price: bundle.price || bundle.bundleStartingPrice, // Use bundleStartingPrice as fallback
                mode: selectedMode,
        ...
        sessionStorage.setItem('bundle', JSON.stringify(cartItems));
                    ...
                    <div className="row gx-5">
                        ...
                        <main className="col-lg-6">
                            <div className="ps-lg-3">
                                ...
                                <button type='button' onClick={()=>handleAddToCart(bundle)} className="theme-btn">Add To Cart</button>
                                </div>
